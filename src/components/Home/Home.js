...
class Home extends Component {
    ...
    curriedEndpoint = type => loadMore => searchTerm => `${API_URL}${type}?api_key=${API_KEY}&language=en-US&page=${loadMore && this.state.currentPage + 1}&query=${searchTerm}`;
    ...
    updateItems = (loadMore, searchTerm) => {
        this.setState(
            ...
            () => {
                ...
                this.fetchItems(!this.state.searchTerm ? this.popularEP(loadMore)('') : this.searchEP(loadMore)(this.state.searchTerm));
            }
        );
    };
